    ...
    <div className="lead-card group relative">
      ...
      {notifications.some((n: any) => !n.is_read && n.leadId === lead.id) && (
        <div className="absolute -top-2 -right-2 z-10">
          <span className="inline-flex items-center justify-center w-5 h-5 bg-red-500 rounded-full text-white text-xs font-bold shadow-md">
            !</span>
        </div>
      )}
      ...
      <div className="mb-4">
        <div className="flex items-start justify-between gap-2 mb-2">
          <div className="flex-1 min-w-0">
            <h3 className="font-bold text-base text-slate-900 truncate group-hover:text-amber-700 transition-colors">
              {lead.clientName}
            </h3>
          </div>
          <div className="flex-shrink-0">
            <Badge color={lifecycle.badge}>
              <span className="text-xs font-semibold">{lifecycle.label}</span>
            </Badge>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2 text-xs text-slate-600 mb-2">
          <span>📍</span>
          <span className="truncate font-medium">{lead.destination || 'TBD'}</span>
          {lead.travelDates?.from && (
            <span className="text-slate-500">•</span>
          )}
          {lead.travelDates?.from && (
            <span className="text-slate-500">
              {new Date(lead.travelDates.from).toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}
            </span>
          )}
        </div>
        ...
        <div className="flex items-center gap-2 flex-wrap">
          {health && (
            <Badge color={healthColor}>
              <span className="text-xs font-semibold">Score: {health.score}%</span>
            </Badge>
          )}
          {(lead as any).islamabadStay && (
            <span className={`text-xs px-2 py-0.5 rounded-full font-semibold ${
              (lead as any).islamabadStay === 'yes' 
                ? 'bg-emerald-100 text-emerald-700' 
                : 'bg-slate-100 text-slate-700'
            }`}>
              {(lead as any).islamabadStay === 'yes' ? '✓ Islamabad' : '✗ No Islamabad'}
            </span>
          )}
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-2 mb-4 pb-4 border-b border-slate-200">
        <div className="min-w-0">
          ...
          <p className="font-bold text-sm text-slate-900">{formatCurrency(lead.budget)}</p>
        </div>
        <div className="min-w-0">
          <p className="text-xs text-slate-500 uppercase tracking-wide font-semibold">Tour Type</p>
          <p className="font-bold text-sm text-slate-900 capitalize">
            {lead.tourType ? lead.tourType.charAt(0).toUpperCase() + lead.tourType.slice(1) : '—'}
          </p>
        </div>
      </div>
      ...
      <div className="space-y-2 mb-4 pb-4 border-b border-slate-200">
        <div className="text-xs">
          ...
          <p className="font-medium text-slate-900 break-all text-xs">{lead.email}</p>
        </div>
        <div className="text-xs">
          <p className="text-slate-500 font-semibold mb-1">📞 Phone</p>
          <p className="font-medium text-slate-900 text-xs">{lead.phone}</p>
        </div>
      </div>
      ...
      <div className="flex flex-col gap-1 text-xs text-slate-500 mb-3">
        ...
        {followUpBadge && (
          <div className="flex justify-between bg-blue-50 p-2 rounded-md text-blue-700 font-semibold">
            <span>⏰ Follow-up</span>
            <span>{followUpBadge}</span>
          </div>
        )}
      </div>
      ...
      {lead.hotelInfo && (
        <div className="mb-3 p-2 rounded-lg bg-amber-50 border border-amber-200">
          <p className="text-xs font-semibold text-amber-900 mb-1">🏨 Hotel Booked</p>
          <p className="text-xs font-medium text-slate-900">{lead.hotelInfo.hotelName}</p>
          <p className="text-xs text-slate-600">{formatCurrency(lead.hotelInfo.roomPrice)}</p>
        </div>
      )}
      ...
      {/* Tab Content */}
      {tab === 'overview' && (
        <div className="mb-3 text-xs space-y-2 max-h-[200px] overflow-y-auto">
          {lead.agentRemarks && (
            <div className="bg-slate-50 p-2 rounded border border-slate-200">
              <p className="font-semibold text-slate-700 mb-1">Notes</p>
              <p className="text-slate-600">{lead.agentRemarks}</p>
            </div>
          )}
          <div className="grid grid-cols-2 gap-2">
            <div>
              <p className="text-slate-500 font-semibold">Source</p>
              <p className="font-medium text-slate-700 capitalize">{lead.source || '—'}</p>
            </div>
            <div>
              <p className="text-slate-500 font-semibold">Status</p>
              <p className="font-medium text-slate-700 capitalize">{lead.status || '—'}</p>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="mb-3 text-xs space-y-2">
          <div className="grid grid-cols-3 gap-2 p-2 bg-slate-50 rounded">
            <div className="text-center">
              <p className="text-slate-600 font-semibold block mb-1">🏨</p>
              <p className="text-slate-700 capitalize font-medium text-xs">
                {availability?.hotel_status || 'N/A'}
              </p>
            </div>
            <div className="text-center">
              <p className="text-slate-600 font-semibold block mb-1">🚗</p>
              <p className="text-slate-700 capitalize font-medium text-xs">
                {availability?.transport_status || 'N/A'}
              </p>
            </div>
            <div className="text-center">
              <p className="text-slate-600 font-semibold block mb-1">👤</p>
              <p className="text-slate-700 capitalize font-medium text-xs">
                {availability?.guide_status || 'N/A'}
              </p>
            </div>
          </div>
          {holdExpiryText && (
            <div className={`text-xs font-semibold p-2 rounded ${
              holdExpiryText === 'Hold expired'
                ? 'bg-red-50 text-red-700'
                : 'bg-amber-50 text-amber-700'
            }`}>
              ⏱️ Hold: {holdExpiryText}
            </div>
          )}
        </div>
      )}
